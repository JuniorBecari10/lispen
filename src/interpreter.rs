...
impl Value {
    fn is_truthy(&self) -> bool {
        match self.clone() {
            ...
            Value::Function { params, body, env } => true,
            Value::Nil => false
        }
    }
}
...
impl Interpreter {
    ...
    pub fn interpret(&mut self, env: &mut env::Environment) {
        if self.exprs.len() == 1 {
            match self.execute(self.exprs[0].clone(), env) {
        ...
        for expr in self.exprs.clone() {
            if self.execute(expr, env).is_none() {
                break;
            }
        }
    }
    ...
    fn execute(&mut self, expr: expr::Expr, env: &mut env::Environment) -> Option<Value> {
        match expr.data {
            ...
            expr::ExprData::List(l, is_quote) => {
                ...
                if is_quote {
                    let mut vec = vec![];
                    for expr in l {
                        vec.push(self.execute(expr, env)?);
                    }
                    ...
                    match l[0].data.clone() {
                        expr::ExprData::Operator(o) => {
                            ...
                            let a = self.execute(l[1].clone(), env)?;
                            let b = self.execute(l[2].clone(), env)?;
                        ...
                        expr::ExprData::Keyword(k) => match k.as_str() {
                            "let" => {
                                ...
                                if let expr::ExprData::Identifier(name) = l[1].data.clone() {
                                    let value = self.execute(l[2].clone(), env)?;
                            ...
                            "if" => {
                                ...
                                if self.execute(l[1].clone(), env)?.is_truthy() {
                                    self.execute(l[2].clone(), env)
                                }
                                else {
                                    match l.get(3).cloned() {
                                        Some(e) => self.execute(e, env),
                                        None => Some(Value::Nil),
                                    }
                                }
                            }
                        ...
                        expr::ExprData::Identifier(name) => {
                            ...
                            if let Value::Function { params, body, env: fn_env } = function {
                                ...
                                for (i, param) in params.iter().enumerate() {
                                    let arg = self.execute(l[i + 1].clone(), env)?;
                                    new_env.define_variable(param.into(), arg.clone());
                                }
                                
                                let res = self.execute(body, &mut new_env)?;
                                return Some(res);
                            }
